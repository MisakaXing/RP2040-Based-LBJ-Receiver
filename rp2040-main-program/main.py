...
def draw_km_post(km, x, y, preferred_scale, row_height, bg_color, spaced=False):
    ...
    if (not spaced and preferred_scale == 2 and len(value) == 6
            ...
            and value[5].isdigit()):
        # Four-digit kilometre posts have exactly 100 px available.  Preserve
        # large digits with 14 px pitch, then tuck a small K below the end.
    ...
    while scale > 1 and x + len(label) * 8 * scale > 320:
        scale -= 1
    # The parsed kilometre post fits at scale 1, including its K suffix.
