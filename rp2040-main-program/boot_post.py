...
class SystemPOST:
    ...
    def check_sys_ver(self, ver, is_es):
        ...
        if is_es == 1:
            self._check_end("WARN", f"v{ver} (Eng Ver)") 
        else:
            self._check_end("OK", f"v{ver} (Release)")
    ...
    def check_sx1276(self, spi_id=0, sck=18, mosi=19, miso=16, cs=17, rst=15):
        ...
        try:
            ...
            cs_pin.value(0)
            buf = bytearray([0x42 & 0x7F, 0x00])
            spi.write_readinto(buf, buf)
            cs_pin.value(1)
            
            ver = buf[1]
            if ver in [0x12, 0x22]: 
                self._check_end("OK", f"SX1276 (v{ver:02X})")
                return True
            else:
                self._check_end("ERR", "NOT FOUND/DEAD")
                return False
        except Exception:
            self._check_end("ERR", "SPI BUS ERROR")
            return False
    ...
    def check_bat(self, bat_adc, bat_en):
        ...
        if volts < 3.3: self._check_end("WARN", f"{volts:.2f}V (Low)")
        else: self._check_end("OK", f"{volts:.2f}V (Good)")

    def check_temp(self, sensor_temp):
        self._check_start("Chk Temp....")
        t = 27 - (sensor_temp.read_u16()*(3.3/65535)-0.706)/0.001721
        if 10 <= t <= 35: self._check_end("OK", f"{t:.1f}C (Norm)")
    ...
    def check_rtc(self, rtc):
        ...
        try:
            ...
            dd, mm, yy = [(r >> 4) * 10 + (r & 0x0F) for r in raw_d]
            if yy < 20: 
                self._check_end("WARN", "Needs Sync!")
                self.rtc_error = True 
            else: 
                self._check_end("OK", f"Valid (20{yy:02})")
        except Exception:
    ...
    def check_sd(self, spi1, sd_cs):
        ...
        try:
            ...
            self._check_end("OK", "Card Present")
        except Exception:
    ...
    def run_all(self, bat_adc, bat_en, sensor_temp, rtc, spi1, sd_cs, buzzer, p_ver, is_es):
        ...
        if not radio_ok:
            ...
            self.tft.draw_gbk(b'SYS HALT - RADIO DEAD', 30, self.y + 6, 0xFFFF, 0xF800, scale=1)
